...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/InFusionX-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        adb wait-for-online
        echo "Found device"
        if (adb shell getprop org.infusion.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-online
    if (adb shell getprop org.infusion.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.infusion.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
